...
Security and quality checks are executed early in the pipeline following the shift-left principle.
...
That way we connect CI to CD in a GitOps way.
...
•	Application-level Prometheus metrics – Only infrastructure-level metrics are collected to demonstrate basic observability.
...
•	learn more and introduce Helm or Kustomize if the project grows to multiple environments

•	change Docker image versioning from Git SHA to semantic versioning for better readability

•   try more "as-code" setups like dashboard-configmap.yaml for grafana dashboard
